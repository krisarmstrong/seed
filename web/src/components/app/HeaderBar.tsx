...
function getFriendlyInterfaceName(name: string, isWifi: boolean): string {
  if (isWifi) {
    ...
    if (match && parseInt(match[0]) > 0) {
      return `Wi-Fi ${parseInt(match[0]) + 1}`;
    }
    return "Wi-Fi";
  }
        ...
        <div className={cn("flex items-center", spacing.gap.tight)}>
          ...
          <div ref={interfaceDropdownRef} className="relative">
            <button
              className={cn(
                iconButtonClass,
                !isWifi &&
                  "ring-2 ring-brand-primary ring-offset-1 ring-offset-surface-raised"
              )}
              onClick={() => setInterfaceDropdownOpen(!interfaceDropdownOpen)}
              aria-label={t(
                "accessibility.selectEthernet",
            ...
            {interfaceDropdownOpen && (
              ...
              >
                <div
                  className={cn(
                    spacing.pad.sm,
                    "border-b border-surface-border bg-surface-base"
                  )}
                >
                    ...
                    interfaces
                      ...
                      .map((iface) => (
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            handleInterfaceSelect(iface.name, false)
                          }
                          className={cn(
                            "w-full text-left",
                            spacing.pad.sm,
          ...
          <div className="relative">
            <button
              ...
              aria-label={t(
                "accessibility.selectWifi",
                "Select Wi-Fi / Survey Mode"
              )}
              title={
                hasWifiInterface
            ...
            >
              ...
              {!hasWifiInterface && (
                <span
                  ...
                  title={t(
                    "interface.noWifiHardware",
                    "No WiFi hardware - Planning mode"
                  )}
                />
              )}
            </button>
