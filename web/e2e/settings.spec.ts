...
test.describe('Settings CRUD Operations', () => {
  ...
  test('should change DNS test hostname and verify save', async ({ page }) => {
    ...
    const dnsInput = page.locator('input[type="text"]').or(
      page.locator('input[placeholder*="hostname" i]')
    );
  ...
  test('should toggle discovery settings and persist', async ({ page }) => {
    ...
    const discoveryToggles = page.locator(
      'input[type="checkbox"]'
    );
  ...
  test('should change performance settings and verify persistence', async ({ page }) => {
    ...
    const speedtestToggle = page.locator('label:has-text("Speedtest"), label:has-text("Speed Test")').locator('input[type="checkbox"]').first();
  ...
  test('should save FAB configuration options', async ({ page }) => {
    ...
    if (hasFabSettings) {
      ...
      if (toggleCount > 0) {
        ...
        if (settings && settings.fabOptions) {
  ...
  test('should handle settings with range inputs', async ({ page }) => {
    ...
    // Set to middle value
    const midValue = (parseInt(min) + parseInt(max)) / 2;
    await firstRange.fill(midValue.toString());
    await page.waitForTimeout(1000);

    // Verify changed
    const newValue = await firstRange.inputValue();
    expect(parseInt(newValue)).toBeGreaterThanOrEqual(parseInt(min));
    expect(parseInt(newValue)).toBeLessThanOrEqual(parseInt(max));
  ...
  test('should validate numeric input boundaries', async ({ page }) => {
    ...
    if (max) {
      // Try to exceed max
      const overMax = parseInt(max) + 1000;
      ...
      expect(resultNum).toBeLessThanOrEqual(parseInt(max) + 1); // Allow small variance
    }

    if (min) {
      // Try to go below min
      const underMin = parseInt(min) - 1000;
      ...
      expect(resultNum).toBeGreaterThanOrEqual(parseInt(min) - 1); // Allow small variance
    }
