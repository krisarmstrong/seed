...
test.describe('Dashboard', () => {
  ...
  test('should display Link Status card', async ({ page }) => {
    const linkCard = page.locator('[data-testid="link-card"]').or(
      page.locator('h3:has-text("Link"), h4:has-text("Link")').first()
    );
  ...
  test('should show help modal', async ({ page }) => {
    // Click help button
    const helpButton = page.getByRole('button', { name: /help/i }).or(
