...
test.describe('Vulnerability Scanning', () => {
  ...
  test('should show scan status', async ({ page }) => {
    ...
    const hasDisabled = await page.getByText(/disabled|not available/i).isVisible().catch(() => false);
  ...
  test('should display vulnerability count if available', async ({ page }) => {
    ...
    const hasNone = await page.getByText(/no vulnerabilit|no issue|clean|secure/i).isVisible().catch(() => false);
