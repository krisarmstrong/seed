 ...
 */
function createMockSurveyWithSamples(surveyId: string, name: string, status: string, sampleCount: number) {
  const samples = [];
...
test.describe('WiFi Survey - Complete User Journey', () => {
  ...
  test('should complete full survey creation flow', async ({ page }) => {
    ...
    const newButton = page.getByRole('button', { name: /\+ New/i }).or(
    ...
    await expect(page.getByRole('heading', { name: /Create New Survey/i })).not.toBeVisible({ timeout: 3000 });
  });
  ...
  test('should open survey view when clicking on survey', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Test Survey/i })).toBeVisible({ timeout: 3000 });
  });
  ...
  test('should add sample points when clicking on floor plan', async ({ page }) => {
    ...
    await expect(page.locator('text=/Click on the floor plan to take a measurement/i')).toBeVisible();
  ...
  test('should show and hide heatmap visualization', async ({ page }) => {
    ...
    const rssiHeatmapButton = page.getByRole('button', { name: /RSSI Heatmap/i });
    ...
    const hideHeatmapButton = page.getByRole('button', { name: /Hide Heatmap/i });
  ...
  test('should show different survey types correctly', async ({ page }) => {
    ...
    const activeSurvey = { ...createMockSurveyWithSamples('survey-2', 'Active Survey', 'created', 0), surveyType: 'active' };
    const throughputSurvey = { ...createMockSurveyWithSamples('survey-3', 'Throughput Survey', 'created', 0), surveyType: 'throughput' };

    await page.route('**/api/survey/list', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ surveys: [passiveSurvey, activeSurvey, throughputSurvey] })
      });
    });
