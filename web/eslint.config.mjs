    ...
    rules: {
      ...
      "no-restricted-syntax": [
        ...
        },
        {
          ...
          message:
            "Use 'bg-surface-raised' or 'bg-surface-base' instead of 'bg-white'. Opacity variants like 'bg-white/20' are allowed for hover effects. See web/THEMING.md",
        },
        {
          // Flag bg-black followed by space or end of string (not opacity variants like bg-black/50)
          selector: "Literal[value=/\\bbg-black(\\s|$)/]",
          message:
            "Use design system tokens instead of 'bg-black'. Opacity variants like 'bg-black/50' are allowed for overlays. See web/THEMING.md",
        },
        {
          selector: "Literal[value=/\\btext-gray-\\d/]",
          message:
            "Use design system tokens (text-text-primary, text-text-secondary, text-text-muted) instead of text-gray-*. See web/THEMING.md",
        },
        {
          selector: "Literal[value=/\\bbg-gray-\\d/]",
          message:
            "Use design system tokens (bg-surface-base, bg-surface-raised, bg-surface-hover) instead of bg-gray-*. See web/THEMING.md",
        },
        ...
        // Small gaps (gap-1, gap-2, gap-3) are allowed for tight layouts
        {
          ...
          message:
            "Use spacing.gap.comfortable or spacing.gap.spacious from theme.ts instead of 'gap-4+'. See web/THEMING.md",
        },
        ...
        {
          selector:
            "JSXElement[openingElement.name.name=/^(span|button|label|h1|h2|h3|h4|h5|h6|p|li|th|td|option)$/] > JSXText[value=/[A-Z][a-z]{2,}/]",
          message:
            "Possible hardcoded UI text detected. Consider using t() from useTranslation for i18n support. See i18n documentation.",
        },
      ],
    },
  },
