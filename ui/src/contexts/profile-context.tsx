...
export function ProfileProvider({ children }: ProfileProviderProps) {
  ...
  const cardSettings = useMemo((): CardSettingsConfig => {
    ...
    return {
      ...
      performance: {
        ...
        speedtest: {
          ...(defaults.performance as { speedtest: { enabled: boolean; autoRunOnLink: boolean } })
            .speedtest,
          ...profileSettings.performance?.speedtest,
        },
        iperf: {
          ...(defaults.performance as { iperf: { enabled: boolean; autoRunOnLink: boolean } })
            .iperf,
      ...
      settingsDebounceTimer.current = setTimeout(async () => {
        ...
        try {
          ...
          if (isMountedRef.current) {
            ...
            logger.error(LogComponents.Profiles, "Failed to save settings to profile", err);
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      try {
        await Promise.all([refreshProfiles(), refreshActiveProfile(), loadBackendDefaults()]);
      } catch (err) {
        logger.error(LogComponents.Profiles, "Failed to load initial profile data", err);
      }
    };
