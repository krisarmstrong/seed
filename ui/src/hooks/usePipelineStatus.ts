...
export interface UsePipelineStatusReturn {
  ...
  updateConfig: (config: PipelineConfig, acknowledgeIdsRisk?: boolean) => Promise<void>;
...
export function usePipelineStatus(
  ...
  const [portIntensityInfo, setPortIntensityInfo] = useState<PortIntensityInfo[]>([]);
  ...
  const handlePipelineEventRef = useRef<((event: PipelineEvent) => void) | null>(null);
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      const phaseNumber = run.currentPhase ? enabledPhases.indexOf(run.currentPhase) + 1 : 0;
  ...
  // Start pipeline
  const startPipeline = useCallback(async (configOverride?: Partial<PipelineConfig>) => {
    try {
      setError(null);
      ...
      if (!response.ok) {
        ...
        throw new Error(text || `Failed to start pipeline: ${response.status}`);
      }
  ...
  const cancelPipeline = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        const text = await response.text();
        throw new Error(text || `Failed to cancel pipeline: ${response.status}`);
      }
  ...
  const updateConfig = useCallback(
    async (newConfig: PipelineConfig, acknowledgeIdsRisk?: boolean) => {
      try {
        ...
        if (!response.ok) {
          const text = await response.text();
          throw new Error(text || `Failed to update config: ${response.status}`);
        }
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (win.__pipelineEventHandlers?.size === 0) {
        win.__pipelineEventHandler = undefined;
        win.__pipelineEventHandlers = undefined;
      }
    };
  }, []); // Empty deps - stableHandler reference is stable

  // Update elapsed time while running
  // Fixes #970, #976: Track interval ID locally, simplified cleanup logic
  useEffect(() => {
