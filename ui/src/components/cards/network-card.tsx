...
import { SimpleBaseCard } from "./base-card";
 ...
 */
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Network card displays IPv4/IPv6 with DHCP timing
export function NetworkCard({
  ...
  const primaryIpRaw =
    ...
    (loading ? tc("status.loading") : hasData ? tr("network.noIp") : tr("network.noData"));
        ...
        <>
          ...
          {hasIpv4 && ipv4 && (
            <>
              ...
              />
              {ipv4.gateway && (
                <CardRow label={tr("network.gateway")} value={ipv4.gateway} wrap mono />
              )}
              {ipv4.dhcpServer && (
                <CardRow label={tr("network.dhcpServer")} value={ipv4.dhcpServer} wrap mono />
              )}
              {ipv4.leaseTime && (
