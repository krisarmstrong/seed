...
import { button, cn, icon, layout, radius, section, spacing } from "../../styles/theme";
...
function _clickToBrokenComponent(): React.JSX.Element {
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        onClick={(): void => setShouldThrow(true)}
        class={cn(
          button.size.md,
...
export const WithRetry: Story = {
  render: () => {
    ...
    function _unstableComponent(): React.JSX.Element {
      if (attemptCount < 2) {
        throw new Error(
...
export const RealWorldCardExample: Story = {
  render: () => {
    function _cardContent({ shouldError = false }: { shouldError?: boolean }): React.JSX.Element {
      if (shouldError) {
