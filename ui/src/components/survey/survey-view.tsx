...
import { AirMapperImport, type ImportOptions } from "./air-mapper-import";
import { type CalibrationPoint, FloorPlanCanvas } from "./floor-plan-canvas";
import { HeatmapLegend } from "./heatmap-legend";
import { HeatmapStats } from "./heatmap-stats";
import { ScaleCalibrationPanel } from "./scale-calibration-panel";
import { SurveyConfigPanel } from "./survey-config-panel";
...
export function SurveyView({ survey: initialSurvey, onClose, onUpdate }: SurveyViewProps) {
  ...
  const handleSaveCalibration = async () => {
    ...
    const rawDistance = Number.parseFloat(calibrationDistance);
      ...
      <div className={cn("max-w-7xl mx-auto", spacing.pad.default, spacing.pad.lg)}>
        ...
        <div className={cn("grid grid-cols-1 lg:grid-cols-3", spacing.gap.spacious)}>
              ...
              {heatmapMetric === null && currentSamples.length > 0 && (
                <div className={cn(spacing.margin.bottom.content, spacing.stack.sm)}>
                  ...
                  {survey.surveyType === "throughput" && (
                    <div>
                      <div
                        className={cn("body-small text-text-muted", spacing.margin.bottom.tight)}
                      >
          ...
          <div className={cn("lg:col-span-1", spacing.stack.default)}>
            ...
            {survey.status === "created" && (
              <div
                className={cn("bg-surface-raised", radius.md, "border border-surface-border pad")}
              >
                <div className={cn(layout.flex.between, spacing.margin.bottom.inline)}>
