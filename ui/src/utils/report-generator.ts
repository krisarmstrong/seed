...
/**
 * Extract recommendations from analysis findings
 */
function extractAnalysisRecommendations(analysis: AnalysisFinding[]): string[] {
...
/**
 * Extract recommendations from failed validation criteria
 */
function extractValidationRecommendations(validation: SurveyValidation | null): string[] {
  if (!validation) {
    return [];
  }
  const recommendations: string[] = [];
  for (const result of validation.results) {
    if (!result.passed && result.failedSampleCount > 0) {
      const recommendation = getRecommendationForCriterion(result.criterionName);
      if (recommendation) {
...
function generateRecommendations(
  ...
  const analysisRecommendations = extractAnalysisRecommendations(analysis);
  const validationRecommendations = extractValidationRecommendations(validation);

  // Combine and remove duplicates
  return [...new Set([...analysisRecommendations, ...validationRecommendations])];
}
