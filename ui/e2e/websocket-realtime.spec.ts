    ...
    test("should show connection status indicator as connected", async ({ page }) => {
      ...
      if (!foundConnected) {
        ...
        const hasData = await page
          .getByText(/up|down|connected|mbps|ms/i)
  ...
  test.describe("Error Handling", () => {
    ...
    test("should handle malformed card_update messages", async ({ page }) => {
      ...
      await page.evaluate(() => {
        ...
        for (const msg of malformedUpdates) {
    ...
    test("should display error message for WebSocket failures", async ({ page }) => {
      ...
      await page.addInitScript(() => {
        ...
        // biome-ignore lint/style/useNamingConvention: WebSocket is a browser global that must use PascalCase
        (window as unknown as { WebSocket: typeof WebSocket }).WebSocket = ((url: string) => {
