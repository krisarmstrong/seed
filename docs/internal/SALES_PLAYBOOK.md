# The Seed (Mustard Seed Networks) - # The Seed Sales Playbook

**Document Version:** 1.0
**Last Updated:** 2025-12-15
**Target Audience:** Sales Team, Channel Partners

---

## Table of Contents

1. [Product Overview](#product-overview)
2. [Value Proposition](#value-proposition)
3. [Target Customers](#target-customers)
4. [Sales Process](#sales-process)
5. [Discovery Questions](#discovery-questions)
6. [Demo Script](#demo-script)
7. [Pricing & Packaging](#pricing--packaging)
8. [Objection Handling](#objection-handling)
9. [Competitive Battle Cards](#competitive-battle-cards)
10. [ROI Calculator](#roi-calculator)
11. [Commission Structure](#commission-structure)
12. [Sales Tools & Resources](#sales-tools--resources)

---

## Product Overview

### What is The Seed?

**The Seed is an AI-powered network diagnostic platform that helps IT teams:**
- Discover and classify network devices automatically
- Diagnose network issues with root cause analysis
- Plan WiFi deployments BEFORE buying hardware (predictive survey)
- Meet compliance requirements (HIPAA, CIS, NIST)
- Detect vulnerabilities and rogue devices

### One-Sentence Pitch

> "The Seed is like having a senior network engineer with AI superpowers on your team 24/7, catching problems before users complain and saving thousands on WiFi deployments."

### Elevator Pitch (30 seconds)

> "Most IT teams waste hours troubleshooting network issues and thousands on WiFi site surveys. The Seed uses AI to tell you EXACTLY what's wrong—'DHCP is slow because your server takes 800ms to respond'—and lets you plan WiFi coverage BEFORE you buy access points. We help healthcare and SMB IT teams save 10+ hours per week on troubleshooting and $2,000-5,000 per WiFi deployment. Our customers say it's like having a senior network engineer who never sleeps."

---

## Value Proposition

### Primary Value Drivers

| Value Driver | Quantifiable Benefit | Customer Impact |
|--------------|----------------------|-----------------|
| **Time Savings** | 10-20 hours/week on troubleshooting | IT staff focus on projects, not firefighting |
| **WiFi Cost Savings** | $2,000-5,000 per deployment | Eliminate expensive on-site surveys |
| **Compliance** | Automated evidence generation | Pass audits faster, reduce consultant fees |
| **Downtime Prevention** | 50%+ of failures predicted 24h early | Happy users, fewer tickets |
| **Security** | Detect rogue devices in 60 seconds | Prevent breaches, meet insurance requirements |

### Pain Points We Solve

**For IT Directors:**
- ❌ **Pain:** "We spend 20+ hours per week troubleshooting network issues"
- ✅ **Solution:** AI root cause analysis tells you exactly what's wrong in seconds

**For Network Engineers:**
- ❌ **Pain:** "WiFi site surveys cost $3,000-5,000 and take weeks to schedule"
- ✅ **Solution:** Predictive WiFi planner shows coverage BEFORE you buy APs

**For HIPAA Officers:**
- ❌ **Pain:** "Proving HIPAA compliance is a nightmare of manual documentation"
- ✅ **Solution:** One-click compliance reports with auditable evidence

**For MSPs:**
- ❌ **Pain:** "We can't afford to send techs on-site for every network issue"
- ✅ **Solution:** Remote diagnostics + fleet management across all client sites

---

## Target Customers

### Ideal Customer Profile (ICP)

**Primary Market: Healthcare (70% of focus)**

| Attribute | Value |
|-----------|-------|
| **Organization Type** | Community hospitals, clinics, medical practices, dental/vision |
| **Size** | 50-500 employees |
| **Annual Revenue** | $10M-$100M |
| **IT Budget** | $200K-$2M |
| **Network Complexity** | 100-2,000 devices, medical devices, guest WiFi, HIPAA segmentation |
| **Pain Points** | Compliance audits, medical device management, WiFi dead zones, rogue detection |
| **Buying Triggers** | HIPAA audit, ransomware scare, WiFi upgrade, new facility |
| **Decision Maker** | IT Director, CIO, BioMed Engineer |
| **Decision Process** | 30-90 day sales cycle, requires budget approval |

**Secondary Market: SMB (20% of focus)**

| Attribute | Value |
|-----------|-------|
| **Organization Type** | Professional services, light manufacturing, retail chains |
| **Size** | 25-250 employees |
| **IT Budget** | $50K-$500K |
| **Pain Points** | No dedicated network engineer, outsourced IT, growth scaling |
| **Decision Maker** | IT Manager, Office Manager, CEO |

**Tertiary Market: MSPs (10% of focus)**

| Attribute | Value |
|-----------|-------|
| **Organization Type** | Managed Service Providers |
| **Client Count** | 10-100 clients |
| **Pain Points** | Scaling remote support, reducing truck rolls, client retention |
| **Decision Maker** | VP of Operations, CTO |

### Negative Indicators (Disqualify)

- ❌ **Less than 25 employees** - Too small, use free tier
- ❌ **No WiFi networks** - Core value prop doesn't apply
- ❌ **Fortune 500 enterprise** - Too complex, not our ICP yet (post-v1.0)
- ❌ **No budget authority** - Can't close, waste of time
- ❌ **"Just looking"** - No pain, no urgency

---

## Sales Process

### Sales Cycle Overview

**Average Sales Cycle:** 30-90 days (varies by tier)

| Stage | Duration | Activities | Exit Criteria |
|-------|----------|------------|---------------|
| **1. Prospecting** | Ongoing | Outbound calls, LinkedIn, referrals, inbound leads | Qualified lead (BANT) |
| **2. Discovery** | 1 week | Discovery call, pain point identification | Pain confirmed, budget exists |
| **3. Demo** | 1 week | Product demo, trial setup | "Wow moment" achieved |
| **4. Trial** | 30 days | Customer uses Premium tier, success check-ins | Measurable value delivered |
| **5. Proposal** | 1 week | Pricing proposal, ROI analysis | Verbal commitment |
| **6. Negotiation** | 1-2 weeks | Legal review, contract negotiation | Signed agreement |
| **7. Onboarding** | 1 week | Implementation, training | Customer success handoff |

### BANT Qualification Framework

**Budget:**
- "What's your annual IT/network budget?"
- "Have you budgeted for network tools this year?"
- **Qualify if:** $10K+ available for software tools

**Authority:**
- "Who else is involved in this decision?"
- "What's your approval process for new software?"
- **Qualify if:** Talking to decision maker or strong influencer

**Need:**
- "What's your biggest network challenge right now?"
- "How much time do you spend troubleshooting per week?"
- **Qualify if:** Clear pain point that costs >$5K/year

**Timeline:**
- "When do you need this solved by?"
- "What happens if you don't fix this soon?"
- **Qualify if:** Decision in <6 months, urgency exists

### Lead Scoring

| Score | Criteria | Action |
|-------|----------|--------|
| **Hot (A)** | All BANT criteria met, urgent pain | Immediate follow-up, fast-track demo |
| **Warm (B)** | 3/4 BANT, pain exists, no urgency | Nurture, provide case studies |
| **Cold (C)** | 2/4 BANT, exploratory | Add to drip campaign, revisit in 3 months |
| **Disqualify (D)** | <2 BANT, no budget/authority | Politely exit, keep in marketing funnel |

---

## Discovery Questions

### Opening Questions (Rapport Building)

1. "Tell me about your network environment—how many sites, devices, users?"
2. "What does a typical day look like for your IT team?"
3. "What tools are you currently using for network management?"

### Pain Point Discovery

**Troubleshooting & Downtime:**
- "How much time does your team spend troubleshooting network issues each week?"
- "When was the last time you had a network outage? How long did it take to fix?"
- "Do you know what's wrong when users complain about slow WiFi, or is it trial and error?"

**WiFi Planning:**
- "How do you plan WiFi coverage for new facilities or expansions?"
- "Have you ever deployed access points and found dead zones you didn't expect?"
- "What did your last WiFi site survey cost? How long did it take?"

**Compliance:**
- "Are you subject to HIPAA, PCI, or other compliance frameworks?"
- "How do you document your network security for audits?"
- "How much time do you spend preparing for compliance audits?"

**Security:**
- "How do you detect unauthorized devices on your network?"
- "Have you had any security incidents in the past year?"
- "How quickly can you identify vulnerable devices after a CVE is published?"

**Cost & Budget:**
- "What's the cost of network downtime to your organization per hour?"
- "How much do you pay for WiFi site surveys, consultants, or external support?"
- "What's your annual budget for network tools and software?"

### Champion Identification

- "Who else would benefit from solving this problem?"
- "If we could solve this, who would be your internal hero?"
- "Who controls the budget for this type of purchase?"

### Urgency & Timeline

- "What happens if you don't solve this in the next 3-6 months?"
- "Is there an upcoming audit, project, or event driving this?"
- "On a scale of 1-10, how urgent is fixing this problem?"

---

## Demo Script

### Pre-Demo Checklist

- ✅ Discovery call completed (know their pain points)
- ✅ Demo environment ready with realistic data
- ✅ Screen sharing tested, backup plan ready
- ✅ Agenda sent to attendees
- ✅ Stakeholders confirmed (decision maker present?)

### Demo Structure (30-45 minutes)

**1. Introduction (5 minutes)**
- Quick recap of discovery call: "You mentioned you spend 15 hours per week troubleshooting and just had a WiFi outage last week, right?"
- Agenda overview: "I'll show you exactly how The Seed solves those problems in about 30 minutes"

**2. The "Wow Moment" (10 minutes)**

**Start with their #1 pain point. Examples:**

**If their pain is troubleshooting:**
- Show AI Root Cause Analysis: "Here's a real issue—DHCP is slow. Most tools stop there. Watch this..."
- Click into insight: "The Seed tells you EXACTLY why: the DHCP server took 850ms to respond, which is 3x your baseline. That's where you start."
- **Wow moment:** "Instead of spending 2 hours checking switches, cables, and configs, you know in 10 seconds it's the DHCP server."

**If their pain is WiFi:**
- Show Predictive WiFi Survey: "Here's a floor plan. You're thinking about adding WiFi. Most people buy 3 APs and hope. Watch this..."
- Draw walls, place virtual APs, generate heatmap
- **Wow moment:** "Before you spend $3,000 on APs, you see exactly where coverage will be. No surprises, no site survey fees."

**If their pain is compliance:**
- Show HIPAA Compliance Report: "Click one button, get an auditor-ready PDF showing how you meet HIPAA §164.312(e)(1)..."
- **Wow moment:** "Instead of 40 hours of manual documentation, it's done in 30 seconds."

**3. Feature Walkthrough (15 minutes)**

Walk through the dashboard showing:
- Network discovery ("Here's every device, automatically classified—printers, cameras, IoT")
- Health score ("One number tells you if your network is healthy")
- Anomaly detection ("This device is using 10x more bandwidth than usual—that's a red flag")
- Vulnerability scanner ("These 5 devices have high-risk CVEs, prioritized by exposure")

**4. Objection Pre-emption (5 minutes)**

Address common objections BEFORE they ask:

- "You might be wondering: 'How is this different from [competitor]?'"
- "Great question—we're the only tool with predictive WiFi planning. No one else lets you plan coverage before buying APs."

**5. Next Steps & Call to Action (5 minutes)**

- "Based on what you've seen, does this solve your [pain point]?"
- **If yes:** "Great! Let's get you started with a 30-day trial of the Premium tier so you can see the WiFi planner in action."
- **If hesitant:** "What questions do you have? What would you need to see to move forward?"

### Demo Do's and Don'ts

| ✅ **DO** | ❌ **DON'T** |
|-----------|--------------|
| Start with their pain point | Give a generic feature tour |
| Show real-world scenarios | Use Lorem Ipsum data |
| Pause for questions frequently | Talk for 20 minutes straight |
| Demonstrate ROI/time savings | Focus on technical specs |
| Tie every feature to their problem | Show features they don't need |
| End with a clear next step | Leave without a commitment |

---

## Pricing & Packaging

### Pricing Tiers

| Tier | Price | Target Customer | Key Features |
|------|-------|-----------------|--------------|
| **Free** | $0 | Hobbyists, home networks | 50 devices, basic monitoring, no AI |
| **Starter** | **$299/year** | Small offices, single-site | 200 devices, device classification, health scoring |
| **Professional** | **$799/year** | SMB, IT teams | Unlimited devices, root cause analysis, anomaly detection |
| **Premium** | **$1,999/year** | WiFi consultants, healthcare | **Predictive WiFi survey**, compliance reports |
| **Enterprise** | **$4,999/year** | MSPs, multi-site | Fleet management, white-label reports, priority support |

### Pricing Strategy

**Positioning:**
- Price 60-75% below competitors (Ekahau $5K, AirMagnet $3K-15K)
- Lead with Premium tier ($1,999) for WiFi consultants
- Land-and-expand with Starter ($299) for SMB, upsell to Pro/Premium

**Discounts:**
- **Annual upfront:** Included (no monthly option yet)
- **Multi-year:** 10% off 2-year, 15% off 3-year
- **Non-profit/Education:** 25% off
- **MSP volume:** See MSP pricing below

**MSP Pricing (Volume Discounts):**

| Sites | Discount | Example: 20 sites @ Premium |
|-------|----------|----------------------------|
| 5-9 sites | 15% off | $1,999 → $1,699/site = $33,980/year |
| 10-19 sites | 25% off | $1,999 → $1,499/site = $29,980/year |
| 20-49 sites | 30% off | $1,999 → $1,399/site = $27,980/year |
| 50+ sites | 40% off | $1,999 → $1,199/site = Custom quote |

### Pricing Objections & Responses

**"That's too expensive"**
- **Probe:** "Too expensive compared to what? What are you spending now on site surveys, consultant hours, or network downtime?"
- **Reframe:** "Let's look at ROI. If you do 2 WiFi surveys per year at $3,000 each, that's $6,000. The Seed is $1,999 and you can do unlimited surveys. You save $4,000 in year one alone."

**"We don't have budget right now"**
- **Probe:** "When does your next budget cycle start?"
- **Reframe:** "What's the cost of NOT fixing this? If your team wastes 10 hours per week at $50/hour, that's $26,000 per year. The Seed costs $799. Even a 10% time savings pays for itself."

**"Can we get a discount?"**
- **Deflect:** "Our pricing is already 60% below competitors like Ekahau. That said, we do offer discounts for annual pre-payment [already included], multi-year commitments [10-15% off], and non-profit/education [25% off]. Which applies to you?"

**"We need to try it first"**
- **Agree:** "Absolutely! That's why we offer a 30-day trial of the Premium tier. You'll see the value before you commit."

---

## Objection Handling

### Common Objections & Responses

**1. "We already have [SolarWinds / PRTG / Nagios / etc.]"**

**Response:**
- "Great tools for monitoring! What we hear from customers is those tools tell you WHAT is broken, but not WHY or HOW to fix it. Do you find yourself spending hours correlating data to find root causes?"
- **Differentiation:** "The Seed's AI tells you exactly why DHCP is slow (server response time) or why WiFi is weak (interference on channel 6). It's the difference between a symptom and a diagnosis."

**2. "We can't justify the cost"**

**Response:**
- "I understand budget is tight. Let's look at what you're spending now:"
  - WiFi site survey: $3,000-5,000 per deployment
  - Network consultant: $150-250/hour
  - IT team troubleshooting: 10-20 hours/week @ $50/hour = $26K-52K/year
- "If The Seed saves just 2 hours per week or eliminates one site survey, it pays for itself. Does that math work for you?"

**3. "We're not ready to buy right now"**

**Response:**
- **Probe urgency:** "I totally understand. What would need to change for this to become a priority?"
- **Nurture:** "No problem. How about I send you a case study from [similar customer] and we reconnect in 3 months?"
- **Trial offer:** "Or, would a no-risk 30-day trial make sense so you can see the value when you're ready?"

**4. "We need to evaluate other options"**

**Response:**
- "Smart approach—you should compare. Which other tools are you looking at?"
- **If they say Ekahau:** "Ekahau is the gold standard for WiFi surveys, no question. The difference is Ekahau requires on-site surveys with expensive hardware ($5K+). The Seed lets you PLAN coverage before you ever buy an AP. It's predictive vs reactive."
- **Battle card:** [See Competitive Battle Cards section]

**5. "Our IT team can build this in-house"**

**Response:**
- "You've got talented people—I believe it! Question: how long would it take to build AI root cause analysis, WiFi RF propagation modeling, and compliance reporting?"
- **Reality check:** "Our team spent 18 months building this. Even if your team could do it in 6 months, that's $100K+ in engineering time. Plus ongoing maintenance. Is that the best use of your team's time, or would you rather focus on strategic projects?"

**6. "We don't need AI, we have experienced engineers"**

**Response:**
- "Your engineers are your biggest asset! The Seed doesn't replace them—it makes them 10x more productive. Instead of spending 2 hours troubleshooting, they get answers in 10 seconds and can focus on projects that drive the business forward."
- "Think of it like spell-check for writers. Good writers don't need spell-check, but it makes them faster and catches mistakes."

**7. "What if it doesn't work for our environment?"**

**Response:**
- "Great question. That's exactly why we offer a 30-day trial. You'll test it in YOUR environment with YOUR network. If it doesn't deliver value, you don't pay. Fair?"

**8. "We need more features before we can commit"**

**Response:**
- **Probe:** "What specific features are you missing?"
- **Roadmap:** "Good news—that's on our roadmap for [quarter]. Can I show you our product roadmap?"
- **Workaround:** "In the meantime, here's how you could achieve that with [alternative approach]."

---

## Competitive Battle Cards

### vs. Ekahau (WiFi Survey Leader)

**When to compete:** WiFi consultants, healthcare facilities planning deployments

**Ekahau Strengths:**
- Industry standard, 60%+ market share
- Professional-grade survey accuracy
- Spectrum analysis (requires hardware)
- Established brand, large customer base

**Ekahau Weaknesses:**
- Requires expensive on-site surveys ($3K-5K per deployment)
- Hardware bundle costs $5,000-20,000
- Reactive (survey AFTER APs deployed to verify)
- No AI insights, no compliance features

**The Seed Advantages:**
- **Predictive survey** - Plan BEFORE buying APs (Ekahau can't do this)
- 60-75% cheaper ($1,999 vs $5,000-20,000)
- Software-only, no hardware required
- AI root cause analysis + compliance reporting (Ekahau doesn't have)

**Positioning:**
> "Ekahau is the Cadillac of WiFi surveys—if you have $5K per deployment and need spectrum analysis, go with them. But if you want to PLAN coverage before you buy APs and save thousands, The Seed's predictive survey is the only option."

**Sales Trap to Avoid:**
- ❌ Don't compete head-to-head on traditional survey accuracy (they'll win)
- ✅ Compete on predictive planning + integrated security + price

---

### vs. AirMagnet Survey PRO

**When to compete:** Enterprise IT, government, large healthcare systems

**AirMagnet Strengths:**
- NetAlly brand (trusted in enterprise)
- Comprehensive wireless analysis
- Integration with NetAlly hardware (AirCheck, LinkSprinter)
- Strong in enterprise/government

**AirMagnet Weaknesses:**
- Complex, steep learning curve
- Expensive ($3,000-15,000 depending on edition)
- Reactive surveys only (no predictive planning)
- Requires Windows (not cross-platform)

**The Seed Advantages:**
- Predictive WiFi planning (AirMagnet doesn't have)
- Modern web UI, easier to learn
- 75-85% cheaper
- Cross-platform (runs on any OS)

**Positioning:**
> "AirMagnet is built for Fortune 500 enterprise. If you have complex requirements and enterprise budget, it's a solid choice. But if you want predictive planning and a tool your whole team can use without a PhD, The Seed is built for you."

---

### vs. Hamina (Closest Competitor)

**When to compete:** WiFi consultants, SMB planning deployments

**Hamina Strengths:**
- Cloud-based, modern UI
- Good RF propagation modeling
- Heatmap prediction (similar to us)
- Automated AP placement

**Hamina Weaknesses:**
- WiFi-only (no network diagnostics, security, compliance)
- Requires manual floor plan creation
- No AI root cause analysis
- Limited integrations

**The Seed Advantages:**
- **All-in-one platform:** WiFi + network diagnostics + security + compliance
- AI-powered root cause analysis (Hamina doesn't have)
- Anomaly detection, vulnerability scanning (Hamina doesn't have)
- HIPAA/CIS compliance reporting (Hamina doesn't have)

**Positioning:**
> "Hamina is great if you ONLY need WiFi planning. But if you want WiFi planning AND network troubleshooting AND compliance reporting in one platform, The Seed is the only option."

**This is Our Toughest Competitor:**
- Hamina's RF modeling is strong—we MUST match their ±10 dB accuracy or we lose
- They're cloud-native, we're on-prem/hybrid (could be strength or weakness)
- Differentiate on AI + security + compliance integration

---

### vs. NetAlly AirMapper

**When to compete:** Field techs, MSPs doing site surveys

**NetAlly Strengths:**
- Hardware-bundled (AirMapper Site Survey app + AirCheck G3 hardware)
- Trusted brand for field techs
- Rugged, portable hardware
- Good for on-site surveys

**NetAlly Weaknesses:**
- Hardware required ($2,500+ for AirCheck G3)
- Reactive surveys only (no predictive planning)
- Limited AI/analytics
- iPad-only (not cross-platform)

**The Seed Advantages:**
- Predictive WiFi planning (NetAlly can't do this)
- Software-only, no hardware required
- Cross-platform (any device)
- AI diagnostics + compliance (NetAlly doesn't have)

**Positioning:**
> "NetAlly is perfect for field techs doing hands-on surveys with hardware. But if you want to PLAN WiFi coverage from your desk without buying $2,500 hardware, The Seed's predictive survey is the way to go."

---

### vs. Free Tools (Wireshark, Nmap, Nagios, etc.)

**When to compete:** Small IT teams, budget-conscious customers

**Free Tools Strengths:**
- Free (can't beat $0)
- Mature, well-documented
- Large community support
- Highly customizable

**Free Tools Weaknesses:**
- Require deep networking expertise
- No AI insights, manual analysis required
- Fragmented (need 5-10 tools to do what The Seed does in one)
- No support, no SLA

**The Seed Advantages:**
- **AI does the analysis for you** (free tools just collect data)
- All-in-one platform (vs. 10 separate tools)
- Professional support + SLA
- Compliance-ready reports (free tools can't generate)

**Positioning:**
> "Free tools are great if you have a senior network engineer with 40 hours per week to analyze data. But if you want AI to tell you exactly what's wrong in 10 seconds, The Seed pays for itself in time savings alone."

**Sales Trap to Avoid:**
- ❌ Don't compete on price (free always wins)
- ✅ Compete on TIME SAVINGS and AI insights

---

## ROI Calculator

### Time Savings ROI

**Assumptions:**
- Average IT hourly rate: $50/hour
- Current troubleshooting time: 10-20 hours/week
- The Seed reduces troubleshooting by 30-50%

| Current State | With The Seed | Annual Savings |
|---------------|----------------|----------------|
| 10 hours/week @ $50/hour | 30% reduction = 3 hours saved/week | **$7,800/year** |
| 15 hours/week @ $50/hour | 40% reduction = 6 hours saved/week | **$15,600/year** |
| 20 hours/week @ $50/hour | 50% reduction = 10 hours saved/week | **$26,000/year** |

**ROI:**
- Starter Tier ($299): **26x-87x ROI**
- Professional Tier ($799): **10x-33x ROI**
- Premium Tier ($1,999): **4x-13x ROI**

---

### WiFi Survey Cost Savings

**Assumptions:**
- Traditional site survey cost: $3,000-5,000 per deployment
- The Seed predictive survey: Included in Premium tier ($1,999/year)

| Deployments/Year | Traditional Cost | The Seed Cost | Savings |
|------------------|------------------|----------------|---------|
| 1 survey/year | $3,000 | $1,999 | **$1,001** |
| 2 surveys/year | $6,000 | $1,999 | **$4,001** |
| 5 surveys/year | $15,000 | $1,999 | **$13,001** |

**ROI:**
- 1 survey: **1.5x ROI**
- 2 surveys: **3x ROI**
- 5 surveys: **7.5x ROI**

---

### Downtime Prevention ROI

**Assumptions:**
- Average cost of network downtime: $5,600/hour (Gartner)
- The Seed prevents 50% of failures with 24h early warning

| Current Downtime | Cost | With The Seed (50% reduction) | Savings |
|------------------|------|-------------------------------|---------|
| 1 hour/month | $67,200/year | $33,600/year | **$33,600** |
| 4 hours/year | $22,400/year | $11,200/year | **$11,200** |
| 1 hour/quarter | $22,400/year | $11,200/year | **$11,200** |

**ROI (vs. Premium tier $1,999):**
- 4 hours/year downtime prevented: **5.6x ROI**
- 1 hour/month prevented: **16.8x ROI**

---

### Compliance Audit ROI

**Assumptions:**
- External consultant for HIPAA audit: $5,000-15,000
- Internal staff time for audit prep: 40-80 hours @ $50/hour = $2,000-4,000
- The Seed automates 50-70% of evidence collection

| Current Audit Cost | With The Seed (50% reduction) | Savings |
|--------------------|-------------------------------|---------|
| $7,000/year (consultant + staff) | $3,500/year | **$3,500** |
| $15,000/year | $7,500/year | **$7,500** |

**ROI (vs. Premium tier $1,999):**
- Small audit: **1.75x ROI**
- Large audit: **3.75x ROI**

---

### Total Economic Impact (TEI) Example

**Scenario: 100-bed community hospital**

| Category | Annual Savings |
|----------|----------------|
| Time savings (15 hours/week → 9 hours) | $15,600 |
| WiFi survey savings (2 deployments) | $4,001 |
| Downtime prevention (4 hours/year) | $11,200 |
| HIPAA audit prep | $3,500 |
| **Total Annual Benefit** | **$34,301** |
| **The Seed Cost (Premium)** | $1,999 |
| **Net Savings** | **$32,302** |
| **ROI** | **17x** |

---

## Commission Structure

### Pure Commission Model Analysis

**Can you pay sales reps 100% commission with no base salary?**

**Short Answer: Yes, but it's risky and limits your talent pool.**

**Pros:**
- ✅ Zero fixed costs—only pay when revenue comes in
- ✅ Highly motivated reps (their income depends on closing deals)
- ✅ Low financial risk for early-stage company
- ✅ Easy to scale up/down (no layoff costs)

**Cons:**
- ❌ Hard to attract top talent (most want base salary for stability)
- ❌ High turnover (reps leave if they don't close deals in first 90 days)
- ❌ Reps may prioritize quick wins over long-term customer success
- ❌ Difficult to enforce activities (can't require daily calls if no base)
- ❌ May attract "coin-operated" reps who don't care about product/mission

**Recommendation:**
- **Pure commission works for:**
  - Inside sales reps (remote, transactional sales)
  - Channel partners / affiliates
  - 1099 contractors (not W2 employees)

- **Better approach for early hires:**
  - Small base ($30K-50K) + high commission (20-30%)
  - Shows you're invested in their success
  - Attracts better talent
  - Allows you to require activities (calls, demos, CRM hygiene)

---

### Commission Structure Options

**Option 1: Pure Commission (No Base)**

**Structure:**
- Base Salary: $0
- Commission: **30-40%** of Annual Contract Value (ACV)
- Target Earnings: $60K-120K (depends on closures)

**Example:**
- Rep closes $200K in ACV
- Commission: $200K × 35% = **$70,000**

**When to Use:**
- 1099 contractors, not W2 employees
- Channel partners / resellers
- Affiliate / referral partners

---

**Option 2: Low Base + High Commission (Recommended)**

**Structure:**
- Base Salary: **$40,000-50,000**
- Commission: **20-25%** of ACV
- OTE (On-Target Earnings): $80K-100K
- Accelerators: 30% commission after 100% of quota

**Example:**
- Base: $40,000
- Quota: $300K ACV
- Commission at quota: $300K × 20% = $60,000
- **Total OTE: $100,000**
- If rep hits 150% of quota ($450K):
  - First $300K: $60,000 (20%)
  - Next $150K: $45,000 (30% accelerator)
  - **Total: $145,000**

**When to Use:**
- W2 inside sales reps (full-time employees)
- Early hires (need to attract talent with stability)
- Allows you to require activities, training, CRM hygiene

---

**Option 3: SaaS Tiered Commission (Industry Standard)**

**Structure:**
- Base Salary: **$50,000-60,000**
- Commission Tiers:
  - 0-70% of quota: 15%
  - 70-100% of quota: 20%
  - 100-150% of quota: 25%
  - 150%+ of quota: 30%
- OTE: $100K-120K
- Quota: $400K-500K ACV

**Example:**
- Base: $60,000
- Quota: $400K
- Rep closes $500K (125% of quota):
  - First $280K (70%): $280K × 15% = $42,000
  - Next $120K (to 100%): $120K × 20% = $24,000
  - Next $100K (to 125%): $100K × 25% = $25,000
- **Total Commission: $91,000**
- **Total Earnings: $151,000**

**When to Use:**
- Scaling sales team (3+ reps)
- Want to reward top performers
- Standard SaaS model (aligns with investor expectations)

---

### Recommended Commission Plan for The Seed

**Year 1: Low Base + High Commission (2 reps)**

| Component | Value |
|-----------|-------|
| Base Salary | $40,000 |
| Commission | 25% of ACV (first year), 10% of renewals |
| Quota | $250,000 ACV (Year 1) |
| OTE | $102,500 ($40K base + $62.5K commission at quota) |
| Accelerator | 30% commission above 100% quota |
| Ramp Period | 90 days (50% quota Month 1, 75% Month 2, 100% Month 3) |

**Why This Works:**
- Low fixed cost ($80K for 2 reps)
- Attractive to talent ($100K+ OTE)
- High commission motivates closures
- Renewal commission incentivizes customer success

---

### Commission by Product Tier

| Tier | Annual Price | Commission (25%) | Why This Matters |
|------|--------------|------------------|------------------|
| Starter | $299 | $75 | Low effort, not worth heavy focus |
| Professional | $799 | $200 | Good for small deals, volume play |
| **Premium** | **$1,999** | **$500** | **Sweet spot—focus here** |
| Enterprise | $4,999 | $1,250 | High effort, longer sales cycle |

**Sales Strategy:**
- Lead with **Premium tier** ($1,999) for healthcare/WiFi consultants
- **80% of focus should be Premium tier** (best commission per effort)
- Upsell Starter → Premium (additional $425 commission)
- Enterprise deals require VP approval (longer cycle, custom contracts)

---

### Clawbacks & Terms

**Clawback Policy:**
- If customer cancels within 90 days: **100% clawback**
- If customer cancels within 180 days: **50% clawback**
- After 180 days: No clawback

**Why Clawbacks Matter:**
- Prevents reps from selling to unqualified customers who churn fast
- Incentivizes proper discovery and qualification
- Aligns rep incentives with long-term customer success

**Payment Terms:**
- Commission paid monthly (not upfront)
- Paid only on COLLECTED revenue (not invoiced revenue)
- Multi-year deals: Commission paid on Year 1 ACV, 10% on renewals

---

### Sales Compensation Examples

**Example 1: Rep Closes $250K Quota (100%)**

| Deal | Tier | ACV | Commission (25%) |
|------|------|-----|------------------|
| Community Hospital | Premium | $1,999 × 30 sites = $59,970 | $14,993 |
| 5 SMB Customers | Professional | $799 × 5 = $3,995 | $999 |
| 8 SMB Customers | Starter | $299 × 8 = $2,392 | $598 |
| MSP (15 sites) | Premium | $1,699 × 15 = $25,485 | $6,371 |
| **Total** | | **$91,842 ACV** | **$22,961 commission** |

**Wait, that's only $91K, not $250K quota?**

True! This is why quotas are STRETCH goals. Let's recalculate for a realistic scenario:

**Example 2: Rep Closes $200K (80% of Quota)**

| Quarter | Deals Closed | ACV | Commission |
|---------|-------------|-----|------------|
| Q1 | 2 Premium, 3 Pro | $10,991 | $2,748 |
| Q2 | 1 Enterprise, 5 Pro | $8,994 | $2,249 |
| Q3 | MSP (20 sites @ Premium) | $27,980 | $6,995 |
| Q4 | 3 Premium, 10 Pro | $13,987 | $3,497 |
| **Total** | | **$61,952 ACV** | **$15,488 commission** |

**Total Earnings Year 1:**
- Base: $40,000
- Commission: $15,488
- **Total: $55,488** (below OTE because only 25% of quota hit)

**This is realistic for Year 1.** Sales takes time to ramp.

---

### Scaling the Sales Team

**Phase 1: Founder-Led Sales (Months 1-6)**
- CEO closes first 10-20 customers
- Learn messaging, objections, ideal customer profile
- Document sales process
- **Cost: $0 (founder time)**

**Phase 2: First Sales Hire (Months 7-12)**
- Hire 1 inside sales rep (low base + high commission)
- Target: $150K-200K ACV Year 1
- **Cost: $40K base + $15K-25K commission = $55K-65K total**

**Phase 3: Scale to 2 Reps (Months 13-18)**
- Hire 2nd rep (same model)
- Rep 1 now at full productivity ($250K+ ACV)
- **Cost: $80K base + $40K-60K commission = $120K-140K total**

**Phase 4: Add Sales Manager (Months 19-24)**
- Hire sales manager when you have 3+ reps
- Sales manager: $80K base + 5% override on team revenue
- **Cost: $80K base + $15K-30K override = $95K-110K**

---

## Sales Tools & Resources

### Required Tools (CRM & Enablement)

| Tool | Purpose | Cost | Priority |
|------|---------|------|----------|
| **HubSpot CRM** | Contact management, pipeline tracking | Free (or $50/mo Pro) | ✅ Critical |
| **Calendly** | Demo scheduling | Free | ✅ Critical |
| **Zoom** | Demos, screen sharing | $15/mo | ✅ Critical |
| **Gong / Chorus** | Call recording, coaching | $100-200/user/mo | 🟡 Phase 2 |
| **DocuSign** | Contract signing | $25/mo | ✅ Critical |
| **Stripe Billing** | Payment processing | 2.9% + 30¢ | ✅ Critical |

### Sales Collateral Checklist

- ✅ Sales deck (15-20 slides)
- ✅ Product demo video (5 minutes)
- ✅ Battle cards (vs. Ekahau, Hamina, AirMagnet, NetAlly)
- ✅ Case studies (healthcare, SMB, MSP)
- ✅ ROI calculator (Excel or web tool)
- ✅ Pricing sheet (one-pager)
- ✅ Compliance overview (HIPAA, CIS, NIST)
- ✅ Email templates (prospecting, follow-up, trial, proposal)
- ✅ Contract template (annual subscription agreement)

### Email Templates

**Prospecting Email (Cold Outreach)**

```
Subject: Save 10 hours/week on network troubleshooting

Hi [First Name],

I noticed [Company] has [trigger: recent WiFi upgrade / HIPAA compliance requirement / new facility opening].

Quick question: How much time does your IT team spend troubleshooting network issues each week?

Most IT directors we work with say 10-20 hours. The Seed's AI tells you exactly what's wrong in seconds—like "DHCP is slow because the server took 850ms to respond"—so your team can fix it in minutes instead of hours.

Would it make sense to see a 10-minute demo? I can show you how a hospital IT director saved 15 hours per week using The Seed.

[Your Name]
```

**Follow-Up After Demo**

```
Subject: Next Steps: The Seed 30-Day Trial

Hi [First Name],

Great talking with you today! As discussed, here's what happens next:

1. **30-Day Premium Trial** - I'll send you access today so you can test the WiFi planner and AI root cause analysis in your environment.

2. **Success Check-In** - Let's schedule a 15-minute call in 2 weeks to make sure you're getting value.

3. **Pricing Proposal** - If you see the value, I'll send a formal proposal at the end of the trial.

Sound good? Click here to pick a time for the success check-in: [Calendly link]

[Your Name]
```

**Trial Conversion (Day 25 of 30)**

```
Subject: 5 days left in your The Seed trial

Hi [First Name],

You're 5 days away from the end of your 30-day trial. Quick check:

- Have you tried the WiFi predictive survey yet?
- Have you seen the AI root cause analysis in action?
- Any questions I can answer?

Based on what you've seen, does The Seed solve your [pain point from discovery call]?

If yes, I'll send over a proposal this week so we can get you up and running without interruption.

[Your Name]
```

---

## Key Metrics & KPIs

### Sales Rep Metrics (Weekly)

| Metric | Target | Measurement |
|--------|--------|-------------|
| Outbound calls | 50+ per week | CRM logged calls |
| Demos scheduled | 5 per week | Calendar bookings |
| Demos delivered | 4 per week (80% show rate) | Completed demos |
| Trials started | 2 per week | Trial activations |
| Proposals sent | 2 per week | Sent proposals |
| Closed deals | 1-2 per month | Signed contracts |

### Pipeline Metrics

| Stage | Conversion Rate | Example |
|-------|-----------------|---------|
| Lead → Qualified | 30% | 100 leads → 30 qualified |
| Qualified → Demo | 50% | 30 qualified → 15 demos |
| Demo → Trial | 60% | 15 demos → 9 trials |
| Trial → Closed | 40% | 9 trials → 3-4 closed deals |
| **Overall Lead → Close** | **3.6-4.8%** | 100 leads → 3-4 customers |

### Customer Acquisition Cost (CAC)

**Formula:** Total Sales & Marketing Cost / New Customers

**Example:**
- Sales rep cost: $55,000/year
- Marketing spend: $20,000/year
- New customers: 25/year
- **CAC: $75,000 / 25 = $3,000 per customer**

**Is This Good?**
- LTV (Lifetime Value) for Premium customer: $1,999 × 3 years × 70% gross margin = **$4,197**
- **LTV:CAC Ratio: 1.4:1** (❌ Too low, need 3:1+)
- **Fix:** Increase conversion rates, reduce sales cycle, or increase pricing/retention

---

## Training & Onboarding

### New Sales Rep Onboarding (Week 1-4)

**Week 1: Product Training**
- Day 1: Company vision, mission, product overview
- Day 2-3: Hands-on product training (become a power user)
- Day 4: Competitive landscape (battle cards)
- Day 5: Shadow founder/senior rep on 3 demos

**Week 2: Sales Process**
- Day 1: CRM training (HubSpot)
- Day 2: Discovery call training (BANT, pain point identification)
- Day 3: Demo script practice (role play)
- Day 4: Objection handling workshop
- Day 5: Contract & pricing training

**Week 3: Practice & Certification**
- Day 1-2: Role play demos (must pass certification)
- Day 3: Create first outbound prospecting list
- Day 4: Send first prospecting emails (manager review)
- Day 5: First live demo (shadowed by manager)

**Week 4: Ramp to Quota**
- Month 1: 50% quota (learning mode)
- Month 2: 75% quota (building pipeline)
- Month 3: 100% quota (full productivity)

---

## Sales Playbook Summary

**TL;DR for Sales Reps:**

1. **Ideal Customer:** Healthcare (100-500 employees), SMB IT teams, WiFi consultants
2. **Primary Pain Points:** Troubleshooting takes too long, WiFi site surveys cost $3K-5K, compliance audits are manual
3. **Value Prop:** AI tells you exactly what's wrong in seconds + predictive WiFi planning saves thousands
4. **Pricing:** Lead with Premium tier ($1,999) for best commission ($500)
5. **Demo:** Start with their pain point (troubleshooting OR WiFi OR compliance), show the "wow moment"
6. **Objections:** ROI calculator beats "too expensive," trial beats "not ready," battle cards beat "evaluating competitors"
7. **Commission:** 25% of ACV (focus on Premium tier for best commission per effort)
8. **Close Rate:** 40% of trials should close (if lower, improve qualification or demo)

---

**Document Owner:** Sales Team
**Next Review:** Quarterly (or after 25 customers closed)

**Questions or Feedback?** Contact [sales-enablement@seed.com]
