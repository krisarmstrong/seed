# The Seed - Brand Guidelines

**Version:** 1.0 (Pre-Launch)
**Last Updated:** December 2025
**Status:** Foundation Document - Assets To Be Created

---

## Executive Summary

This document defines the visual identity, voice, and brand standards for **The Seed** by **Mustard Seed Networks**.

**Brand Essence:**
- **Core Metaphor:** From a tiny seed, a mighty network grows
- **Personality:** Approachable, intelligent, growth-oriented, trustworthy
- **Promise:** Complex network diagnostics made simple through AI

**Status:** Pre-launch. Many visual assets (logo, photography, illustrations) are yet to be created. This document establishes the foundation and direction for brand development.

---

## Table of Contents

1. [Brand Story & Positioning](#brand-story--positioning)
2. [Visual Identity](#visual-identity)
3. [Logo Guidelines](#logo-guidelines)
4. [Color Palette](#color-palette)
5. [Typography](#typography)
6. [Iconography](#iconography)
7. [Photography & Imagery](#photography--imagery)
8. [Voice & Tone](#voice--tone)
9. [Messaging Framework](#messaging-framework)
10. [Digital Presence](#digital-presence)
11. [Marketing Collateral](#marketing-collateral)
12. [Brand Do's and Don'ts](#brand-dos-and-donts)

---

## Brand Story & Positioning

### Origin Story

**The Parable of the Mustard Seed:**

> "The kingdom of heaven is like a mustard seed, which a man took and planted in his field. Though it is the smallest of all seeds, yet when it grows, it is the largest of garden plants and becomes a tree, so that the birds come and perch in its branches."
> — Matthew 13:31-32

**Our Interpretation:**

From the smallest diagnostic scan, comprehensive network insight grows. A single seed of data planted in the right soil (AI analysis) becomes a mighty tree of actionable intelligence.

**Founder Story:**

Kris Armstrong spent years working in healthcare IT, watching network admins waste hours troubleshooting mysterious network issues and thousands of dollars on WiFi site surveys. He saw expensive enterprise tools that were too complex for SMBs and healthcare teams, and consultant-focused tools that priced out smaller organizations.

The Seed was born from a simple belief: network diagnostics should be **simple, affordable, and intelligent**. Not just data dumps, but actual answers. Not just for Fortune 500 enterprises, but for the hospitals, clinics, schools, and small businesses that make up the backbone of our communities.

---

### Brand Positioning

**Category:** AI-Powered Network Diagnostic Platform

**Positioning Statement:**
> "For healthcare and SMB IT teams who waste hours troubleshooting network issues and thousands on WiFi site surveys, The Seed is an AI-powered diagnostic platform that tells you exactly what's wrong in seconds and lets you plan WiFi coverage before buying access points. Unlike SolarWinds or Ekahau, The Seed combines network diagnostics, WiFi planning, and compliance automation in one affordable platform starting at $299/year."

**Target Audiences:**
1. **Primary:** Healthcare IT (hospitals, clinics, medical practices)
2. **Secondary:** SMB Network Admins (50-500 employees)
3. **Tertiary:** WiFi Consultants (budget-conscious, smaller projects)

**Brand Pillars:**

1. **Simplicity**
   - Complex networks made simple
   - 30-minute setup, not weeks
   - Plain English, not technical jargon

2. **Intelligence**
   - AI tells you WHY and HOW TO FIX
   - Predictive, not just reactive
   - Learns and improves over time

3. **Growth**
   - Start small (Free tier), grow as you need
   - From tiny seeds, mighty networks grow
   - Scalable from 10 devices to 10,000

4. **Trust**
   - Self-hosted (your data never leaves)
   - Healthcare-grade security (HIPAA compliant)
   - Transparent pricing, no vendor lock-in

---

## Visual Identity

### Brand Personality

If The Seed were a person, they would be:

- **Approachable:** Not intimidating like enterprise software
- **Intelligent:** Thoughtful, insightful, explains things clearly
- **Optimistic:** Focused on growth and possibilities
- **Trustworthy:** Reliable, secure, keeps promises
- **Down-to-earth:** No corporate jargon, no BS

**Visual Metaphors:**
- **Seeds & Growth:** Germination, roots, sprouting, trees
- **Nature & Organic:** Natural, sustainable, life-giving
- **Light & Clarity:** Bringing clarity to complex problems
- **Simplicity & Minimalism:** Clean, uncluttered, focused

---

### Visual Style Direction

**Overall Aesthetic:**
- Modern but warm (not cold/sterile like enterprise software)
- Clean and uncluttered (white space is good)
- Natural elements (organic shapes, earth tones)
- Professional but approachable (not stuffy)

**Avoid:**
- Generic tech stock photos (servers in blue light, etc.)
- Overly corporate (stiff, formal, intimidating)
- Childish/playful (we're B2B, not consumer app)
- Dark mode only (we're about clarity and light)

---

## Logo Guidelines

### Logo Status: TO BE DESIGNED

**Logo Concepts to Explore:**

#### Concept 1: Sprouting Seed Icon
- Simple line drawing of a seed with a small sprout emerging
- Could evolve into different growth states (seed → sprout → sapling → tree) for different tiers or product areas
- Minimalist, memorable, scalable

#### Concept 2: Network + Seed Hybrid
- Seed shape formed by network nodes/connections
- Represents: network analysis growing from a small seed
- More technical, clearly "network tool"

#### Concept 3: Wordmark with Subtle Seed Detail
- Clean, modern wordmark "The Seed"
- Subtle seed icon integrated (e.g., dot over "i" replaced with seed, or "o" in "Seed" becomes seed shape)
- Professional, versatile

**Logo Requirements:**
- Scalable (works at 16px favicon, works on billboard)
- Monochrome version (for black/white printing)
- Icon-only version (for app icon, social media profile)
- Horizontal version (for headers, email signatures)
- Vertical/stacked version (for narrow layouts)

**File Formats Needed:**
- SVG (vector, primary format)
- PNG (transparent background, various sizes: 512px, 256px, 128px, 64px, 32px, 16px)
- PDF (for print materials)

**Clear Space:**
- Minimum clear space around logo: Equal to height of "S" in "Seed"
- Never place text or other elements within clear space

**Minimum Size:**
- Digital: 24px height (for legibility)
- Print: 0.5 inches height

---

### Logo Usage Do's and Don'ts

**DO:**
✅ Use official logo files (never recreate from scratch)
✅ Maintain aspect ratio (never stretch or squash)
✅ Use on solid backgrounds with sufficient contrast
✅ Place on clean, uncluttered backgrounds

**DON'T:**
❌ Change logo colors (except approved variations)
❌ Add effects (shadows, glows, gradients) to logo
❌ Rotate or flip logo
❌ Place logo on busy or low-contrast backgrounds
❌ Use outdated logo versions (always use latest)

---

## Color Palette

### Primary Colors

**Seed Green (Primary Brand Color)**
- Hex: `#2D7A3E` (to be finalized)
- RGB: `45, 122, 62`
- Use: Primary CTA buttons, links, accents, logo (primary version)
- Meaning: Growth, health, life, prosperity

**Soil Brown (Secondary)**
- Hex: `#5C4033` (to be finalized)
- RGB: `92, 64, 51`
- Use: Headers, grounding elements, secondary accents
- Meaning: Foundation, stability, earth

**Sky Blue (Tertiary)**
- Hex: `#4A90E2` (to be finalized)
- RGB: `74, 144, 226`
- Use: Information, calm, trust elements
- Meaning: Clarity, air, openness

---

### Neutral Colors

**Neutrals (Grayscale)**
- **Midnight:** `#1A1A1A` - Headlines, body text
- **Charcoal:** `#333333` - Secondary text
- **Slate:** `#666666` - Tertiary text, captions
- **Silver:** `#999999` - Disabled states, borders
- **Cloud:** `#E5E5E5` - Backgrounds, dividers
- **Snow:** `#F8F8F8` - Page backgrounds
- **Pure White:** `#FFFFFF` - Cards, modals

---

### Functional Colors

**Success (Positive States)**
- Green: `#28A745`
- Use: Success messages, healthy status, checkmarks

**Warning (Caution States)**
- Amber: `#FFC107`
- Use: Warnings, moderate priority alerts

**Error (Negative States)**
- Red: `#DC3545`
- Use: Errors, critical alerts, destructive actions

**Info (Informational)**
- Blue: `#17A2B8`
- Use: Informational messages, tips, help text

---

### Color Accessibility

**WCAG 2.1 AA Compliance Required:**
- Text contrast ratio minimum: 4.5:1 (normal text), 3:1 (large text 18pt+)
- Always test color combinations for accessibility
- Provide non-color indicators (icons, labels) in addition to color

**Tools:**
- WebAIM Contrast Checker: https://webaim.org/resources/contrastchecker/
- Figma plugin: Stark (accessibility checker)

---

### Color Usage Guidelines

**Primary Actions:**
- Use **Seed Green** for primary CTAs ("Get Started", "Download", "Sign Up")
- Reserve for most important action on page (only one primary CTA per section)

**Secondary Actions:**
- Use **Soil Brown** or outlined buttons (green border, white fill)

**Backgrounds:**
- Prefer **Snow** (#F8F8F8) for page backgrounds
- Use **Pure White** for content cards, modals
- Avoid pure black backgrounds (use Midnight #1A1A1A instead)

**Text:**
- Body text: **Midnight** on **Snow/White** backgrounds
- Headings: **Midnight** (can use **Seed Green** for emphasis)
- Links: **Seed Green** (with underline on hover)
- Captions/metadata: **Slate**

---

## Typography

### Typeface Selection: TO BE FINALIZED

**Recommended Directions:**

#### Option 1: Modern Sans-Serif (Recommended)
- **Headings:** Inter, Poppins, or DM Sans (Google Fonts)
- **Body:** Inter or System Font Stack
- **Code/Terminal:** JetBrains Mono or Fira Code

**Why:** Clean, modern, highly legible, free, web-optimized

#### Option 2: Approachable Sans-Serif
- **Headings:** Nunito, Quicksand
- **Body:** Open Sans, Lato
- **Code:** Source Code Pro

**Why:** Friendly, approachable, less "corporate"

#### Option 3: System Font Stack (Pragmatic)
- Use native system fonts for performance
```css
font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Roboto',
             'Helvetica Neue', Arial, sans-serif;
```
**Why:** Fast, familiar, no font loading delay

---

### Typography Scale

**Recommended Scale (Responsive):**

| Element | Desktop | Mobile | Weight |
|---------|---------|--------|--------|
| Hero Heading (H1) | 48px / 3rem | 32px / 2rem | Bold (700) |
| Page Title (H2) | 36px / 2.25rem | 28px / 1.75rem | Bold (700) |
| Section Heading (H3) | 28px / 1.75rem | 24px / 1.5rem | Semibold (600) |
| Subsection (H4) | 20px / 1.25rem | 18px / 1.125rem | Semibold (600) |
| Body Large | 18px / 1.125rem | 16px / 1rem | Regular (400) |
| Body | 16px / 1rem | 16px / 1rem | Regular (400) |
| Body Small | 14px / 0.875rem | 14px / 0.875rem | Regular (400) |
| Caption | 12px / 0.75rem | 12px / 0.75rem | Regular (400) |
| Button | 16px / 1rem | 14px / 0.875rem | Medium (500) |
| Code | 14px / 0.875rem | 14px / 0.875rem | Regular (400) |

**Line Height:**
- Headings: 1.2-1.3 (tight)
- Body text: 1.6-1.8 (comfortable for reading)
- Captions: 1.4-1.5

**Letter Spacing:**
- Headings: -0.02em (slightly tighter)
- Body: 0 (default)
- All caps text: 0.05em (looser for legibility)

---

### Typography Guidelines

**DO:**
✅ Use a clear hierarchy (readers should know what's important)
✅ Limit to 3 weights (Regular, Medium, Bold - avoid too many)
✅ Use adequate line height (1.6+ for body text)
✅ Align text left for readability (centered for short headlines only)

**DON'T:**
❌ Use more than 2 font families on one page
❌ Use fancy/decorative fonts (we're B2B, not a wedding invitation)
❌ Set body text in ALL CAPS (hard to read)
❌ Use font sizes smaller than 14px for body text (accessibility)

---

## Iconography

### Icon Style

**Direction:** Line icons (outline style) with consistent stroke weight

**Characteristics:**
- **Stroke weight:** 2px (1.5px for small sizes)
- **Style:** Rounded corners (not sharp angles)
- **Complexity:** Simple, clear, recognizable at small sizes
- **Color:** Inherit from context (not hardcoded colors in SVG)

**Recommended Icon Sets:**
- **Lucide Icons:** https://lucide.dev (clean, modern, open source)
- **Heroicons:** https://heroicons.com (Tailwind CSS, free)
- **Feather Icons:** https://feathericons.com (minimal, consistent)

**Custom Icons:**
- Use same stroke weight and corner radius as chosen icon set
- 24x24px artboard (scales well to 16px, 32px, 48px)
- Export as SVG (inline or sprite sheet)

---

### Icon Usage

**Navigation:**
- Use icons + labels (not icons alone - aids recognition)
- Active state: Fill icon or change color to Seed Green

**Features:**
- Illustrate features with icons (WiFi waves, network nodes, shield, etc.)
- Pair with short headline and description

**Buttons:**
- Leading icon (left side): ✅ Download, ⚙️ Settings
- Avoid trailing icons unless indicating direction (→ Next)

**Status Indicators:**
- ✅ Green checkmark: Success, healthy, completed
- ⚠️ Yellow warning triangle: Warning, needs attention
- ❌ Red X: Error, failed, critical
- ℹ️ Blue info circle: Informational, help

---

## Photography & Imagery

### Photography Style: TO BE CREATED

**Direction:**
- Authentic, not stock photo-y
- Real people in real environments (hospitals, offices, server rooms)
- Natural lighting (not overly staged)
- Diverse representation (age, gender, ethnicity)

**Subject Matter:**
- Healthcare workers using technology
- IT professionals troubleshooting networks
- Modern office environments
- Clean, organized server rooms (not chaotic cable nightmares)

**Avoid:**
- Cheesy stock photos (person pointing at screen with fake smile)
- Generic server room with blue lighting
- Overly "corporate" (stiff, formal)
- Outdated technology (CRT monitors, old laptops)

---

### Illustrations

**Direction:** Simple, clean, explanatory (not decorative)

**Use Cases:**
- Explain complex concepts (WiFi heatmaps, network topology)
- Onboarding/empty states ("No devices found yet - run your first scan!")
- Error states ("Connection lost - check your internet")

**Style:**
- Line art (consistent with icon style)
- Minimal color (Seed Green + one accent)
- Geometric shapes (avoid overly organic/hand-drawn)

**Recommended Illustration Sets (as starting point):**
- **unDraw:** https://undraw.co (free, customizable color)
- **Storyset:** https://storyset.com (free, animated option)

---

## Voice & Tone

### Brand Voice

**Core Attributes:**

1. **Clear**
   - Plain English, not jargon
   - Short sentences, active voice
   - Explain technical concepts simply

   **Example:**
   - ❌ "Utilize our heuristic algorithms to ascertain root causality of network degradation"
   - ✅ "The Seed tells you what's wrong with your network and how to fix it"

2. **Confident**
   - We know our stuff, we're not guessing
   - Make bold claims (but back them up)
   - Don't hedge with "maybe", "probably", "hopefully"

   **Example:**
   - ❌ "The Seed might be able to help identify some network issues"
   - ✅ "The Seed finds network issues in seconds"

3. **Helpful**
   - Focus on solving customer problems
   - Offer guidance, not just features
   - Anticipate questions and answer them

   **Example:**
   - ❌ "Advanced SNMP v3 support"
   - ✅ "Connect to managed switches securely with SNMP v3 (here's how to configure it)"

4. **Human**
   - Write like a person, not a corporation
   - Use "we", "you", "us" (not "one" or passive voice)
   - Inject personality (but stay professional)

   **Example:**
   - ❌ "Upon installation, configuration parameters must be specified"
   - ✅ "When you first install The Seed, we'll walk you through a quick setup"

---

### Tone Variations

**Tone changes based on context, but voice stays consistent.**

#### Marketing Website (Optimistic, Inspiring)
- Emphasize transformation ("From hours of troubleshooting to seconds")
- Paint a vision ("Imagine knowing exactly what's wrong the moment a user reports a problem")
- Be aspirational but believable

#### Product UI (Clear, Efficient)
- Be concise (users are trying to get work done)
- Front-load important info ("3 vulnerabilities found" not "We found some issues")
- Use progressive disclosure (show summary, click for details)

#### Documentation (Patient, Thorough)
- Step-by-step instructions (numbered lists)
- Assume reader is smart but might not have context
- Provide examples ("Here's what a working config looks like")

#### Error Messages (Calm, Constructive)
- Explain what happened in plain English
- Tell user how to fix it (not just "Error 500")
- Don't blame user ("We couldn't connect" not "You entered invalid credentials")

#### Support (Empathetic, Solution-Oriented)
- Acknowledge frustration ("I understand this is blocking your work")
- Set expectations ("I'll investigate and get back to you within 4 hours")
- Follow up ("Just checking in - is this resolved?")

---

### Writing Guidelines

**Headings:**
- Front-load keywords (SEO + scannability)
- Use action verbs ("Plan WiFi Coverage Before Buying APs" not "WiFi Planning Feature")
- Ask questions ("Wasting Hours on Network Issues?" for problem/agitation)

**Body Copy:**
- One idea per paragraph
- Use bullets for lists (easier to scan than paragraphs)
- Bold key phrases (helps skimmers)

**CTAs (Calls to Action):**
- Be specific ("Download The Seed for macOS" not "Download")
- Use first person when appropriate ("Start My Free Trial" not "Start Free Trial")
- Create urgency without being pushy ("Try It Free Today" not "HURRY LIMITED TIME OFFER!!!")

**Word Choice:**
- **Use:** Simple, everyday words
- **Avoid:** Corporate jargon, buzzwords, overused tech terms
  - ❌ Leverage, utilize, synergy, paradigm, best-of-breed, enterprise-grade
  - ✅ Use, tool, together, approach, professional, reliable

---

## Messaging Framework

### Tagline Options (TO BE FINALIZED)

**Contenders:**
1. "From a tiny seed, a mighty network grows"
2. "Network diagnostics made simple"
3. "See what's wrong. Fix it fast."
4. "AI-powered network intelligence"
5. "Know your network inside and out"

**Testing Criteria:**
- Memorable (can you repeat it after hearing once?)
- Differentiated (could this apply to competitors?)
- Meaningful (does it communicate value?)

---

### Value Propositions by Audience

#### Healthcare IT
**Headline:** "HIPAA Compliance Without the Headache"

**Subheadline:** "Scan your network for vulnerabilities, plan WiFi coverage for new wings, and generate audit-ready reports in minutes - not weeks."

**Proof Points:**
- "Find rogue DHCP servers before they cause an outage"
- "Prove your network is segmented for HIPAA compliance"
- "Plan WiFi for new clinic buildouts without expensive consultants"

---

#### SMB Network Admin
**Headline:** "Stop Guessing. Start Knowing."

**Subheadline:** "AI tells you exactly what's wrong with your network and how to fix it - no more googling error messages at 2am."

**Proof Points:**
- "Discover every device on your network in 60 seconds"
- "Root cause analysis in plain English, not tech jargon"
- "$799/year for unlimited devices - no hidden fees"

---

#### WiFi Consultant
**Headline:** "Plan WiFi Before You Survey"

**Subheadline:** "Predictive WiFi planning lets you design coverage and estimate AP count BEFORE going on-site. Save time, impress clients, win more projects."

**Proof Points:**
- "Generate professional heatmaps and reports"
- "$1,999/year - 60% cheaper than Ekahau"
- "Works on macOS and Linux, not just Windows"

---

### Elevator Pitch (30 seconds)

> "The Seed is an AI-powered network diagnostic tool for healthcare and small businesses. It combines WiFi planning, network monitoring, and vulnerability scanning in one affordable platform. Instead of just showing you data, The Seed tells you what's wrong and how to fix it. It's 60-90% cheaper than tools like Ekahau and SolarWinds, and takes 30 minutes to set up instead of weeks. We're pre-launch, targeting hospitals and clinics first."

---

## Digital Presence

### Website (mustardseednetworks.com)

**Homepage Structure:**

1. **Hero Section:**
   - Headline: Value prop
   - Subheadline: Supporting detail
   - CTA: "Download for macOS" / "Download for Linux" / "See Pricing"
   - Visual: Screenshot of app or demo video

2. **Problem Section:**
   - "Wasting hours troubleshooting network issues?"
   - "Spending $5K+ on WiFi site surveys?"
   - "Struggling with HIPAA compliance?"

3. **Solution Section:**
   - "The Seed tells you what's wrong in seconds"
   - 3 key features with icons and short descriptions

4. **How It Works:**
   - Step 1: Install in 30 seconds
   - Step 2: Run discovery scan
   - Step 3: Get AI-powered insights

5. **Pricing:**
   - 5 tiers in cards (Free, Starter, Professional, Premium, Enterprise)

6. **Social Proof:**
   - Testimonials (when we have them)
   - Logos of customers (when we have permission)

7. **CTA Section:**
   - "Start diagnosing your network in 30 minutes"
   - Download buttons

**Footer:**
- About Us
- Documentation
- Pricing
- Blog
- Community (Discord)
- Support
- Legal (Privacy, Terms)

---

### Social Media

**LinkedIn (Primary B2B Channel):**
- Company Page: "Mustard Seed Networks"
- Kris Armstrong personal profile (thought leadership)
- Content: Product updates, healthcare IT tips, customer stories
- Tone: Professional, informative

**Twitter/X:**
- @MustardSeedNet (to be registered)
- Content: Quick tips, industry news, product updates
- Tone: Casual but professional, conversational

**YouTube:**
- Channel: "Mustard Seed Networks"
- Content: Tutorials, demos, webinars
- Thumbnails: Consistent style (brand colors, clean text, screenshots)

**Reddit:**
- Participate (don't spam) in:
  - r/networking
  - r/sysadmin
  - r/healthIT
  - r/WiFi
- Tone: Helpful, not salesy (answer questions, provide value)

---

### Email

**From Name:** "The Seed Team" or "Kris Armstrong (Mustard Seed Networks)"

**Subject Lines:**
- Be clear, not clever (unless very confident)
- Front-load value ("New: Predictive WiFi Planning" not "You're going to love this")
- Keep under 50 characters (mobile preview)

**Email Design:**
- Plain text preferred for transactional/personal emails
- HTML for newsletters/announcements (branded template)
- Always include plain text version (accessibility + spam filters)

**Signature:**
```
Kris Armstrong
Founder, Mustard Seed Networks
The Seed - AI-Powered Network Diagnostics
https://mustardseednetworks.com
```

---

## Marketing Collateral

### One-Pager (Sales Sheet)

**Front Side:**
- Logo + tagline
- Headline: "Network Diagnostics Made Simple"
- 3-4 key features with icons and 1-sentence descriptions
- Pricing tiers (simplified: Free, Pro $799, Premium $1,999, Enterprise $4,999)
- CTA: QR code → website

**Back Side:**
- "How The Seed Compares" - competitive comparison table
- Customer testimonial (when available)
- "Get Started" - download links or contact info

**Specs:**
- 8.5" x 11" (folds to 8.5" x 5.5")
- Full color
- Matte finish (not glossy - easier to write on)

---

### Business Cards

**Front:**
- Logo
- Name: Kris Armstrong
- Title: Founder
- Email: kris.armstrong@mustardseednetworks.com
- Phone: (TBD)

**Back:**
- Tagline: "From a tiny seed, a mighty network grows"
- Website: mustardseednetworks.com
- QR code → website

**Specs:**
- Standard size: 3.5" x 2"
- Thick stock (32pt)
- Rounded corners (optional but friendlier)

---

### Slide Deck (Conference/Demo)

**Template:**
- Title slide: Logo, presentation title, presenter name
- Section dividers: Full-bleed background color (Seed Green or Soil Brown), white text
- Content slides: White background, minimal text, large visuals
- Footer: Logo + slide number

**Fonts:**
- Headings: 36pt minimum
- Body: 24pt minimum (readable from back of room)

**Images:**
- Full-bleed or clean padding
- No pixelated or stretched images

---

### Video (Demo/Tutorial)

**Intro (5 seconds):**
- Animated logo + tagline
- Music: Upbeat but not distracting (royalty-free: Epidemic Sound, Artlist)

**Content:**
- Screen recording: High resolution (1920x1080 minimum)
- Voiceover: Clear, conversational (not monotone robot)
- Captions: Always (accessibility + silent autoplay on social)

**Outro (5 seconds):**
- CTA: "Download The Seed at mustardseednetworks.com"
- Logo

---

## Brand Do's and Don'ts

### DO: ✅

**Visual:**
✅ Use official brand colors and fonts
✅ Maintain consistent visual style across all materials
✅ Test designs for accessibility (color contrast, text size)
✅ Use high-quality images (no pixelated or blurry)
✅ Give content room to breathe (white space is good)

**Messaging:**
✅ Speak to customer pain points (empathy first)
✅ Be specific ("Save 10 hours/week" not "Save time")
✅ Back up claims with proof ("90% of users..." not "Many users...")
✅ Use active voice ("The Seed finds issues" not "Issues are found")
✅ Write for humans, not algorithms (SEO is secondary to clarity)

**Tone:**
✅ Be confident but not arrogant
✅ Be helpful, not condescending
✅ Be professional, not stuffy
✅ Acknowledge when competitors do something well (builds trust)

---

### DON'T: ❌

**Visual:**
❌ Use random fonts or colors (stick to brand guidelines)
❌ Stretch or distort logos or images
❌ Use clipart or cheesy stock photos
❌ Overuse effects (gradients, shadows, glows)
❌ Ignore accessibility (we serve healthcare - inclusivity matters)

**Messaging:**
❌ Use jargon or buzzwords to sound smart ("leverage our paradigm-shifting solution")
❌ Make claims you can't back up ("The #1 network tool" - says who?)
❌ Be vague ("We help you succeed" - how?)
❌ Talk about features without benefits ("SNMP v3 support" - so what?)
❌ Be negative about competitors ("SolarWinds sucks" - unprofessional)

**Tone:**
❌ Be snarky or sarcastic (jokes don't translate well in text)
❌ Use fear tactics ("Your network is at risk!" - icky)
❌ Be overly casual ("Hey dude, check this out!" - we're B2B)
❌ Use exclamation points excessively!!! (one is enough!)
❌ ALL CAPS (feels like shouting)

---

## Brand Evolution

**This is version 1.0 - a foundation, not a final answer.**

As we grow, our brand will evolve. Expect updates:
- After logo is designed (Q1 2026)
- After first 100 customers (gather feedback)
- After major product milestones (v2.0, etc.)
- Annually (refresh to stay modern)

**Guardrails:**
- Don't change brand drastically (confuses customers)
- Evolve, don't reinvent (unless major pivot)
- Test changes with customers first
- Document changes in this guide

---

## Resources & Tools

### Design Tools
- **Figma:** UI/UX design, prototyping, collaboration (free tier available)
- **Canva:** Marketing materials, social media graphics (free tier available)
- **Adobe Creative Suite:** Professional design (Photoshop, Illustrator, InDesign)

### Stock Assets (Free)
- **Photos:** Unsplash, Pexels
- **Illustrations:** unDraw, Storyset
- **Icons:** Lucide, Heroicons, Feather Icons
- **Fonts:** Google Fonts
- **Colors:** Coolors (palette generator)

### Accessibility Testing
- **WebAIM Contrast Checker:** Color contrast ratios
- **WAVE:** Web accessibility evaluation
- **Lighthouse:** Chrome DevTools (accessibility audit)

### Brand Asset Repository (TO BE CREATED)
- Create shared folder (Google Drive or Dropbox):
  - `/logos` - All logo variations and formats
  - `/colors` - Color swatches (ASE for Adobe, CSS file)
  - `/fonts` - Font files and installation instructions
  - `/icons` - Icon library (SVG, PNG)
  - `/templates` - Slide deck, business card, email signature templates
  - `/guidelines` - This document (PDF version)

---

## Approval Process

**Minor Brand Usage (Day-to-Day):**
- Kris Armstrong approves (or delegates to Marketing Manager when hired)
- Examples: Social media posts, blog graphics, support materials

**Major Brand Changes:**
- Kris Armstrong final approval
- Examples: Logo redesign, color palette change, tagline change

**External Brand Usage (Partners, Press):**
- Require approval before publication
- Provide brand assets and guidelines
- Review before launch (ensure compliance)

---

## Contact

**Brand Questions:**
- Email: kris.armstrong@mustardseednetworks.com
- Subject: "Brand Guidelines Question"

**Request Brand Assets:**
- Email same address with subject "Brand Asset Request"
- Include: What asset, what format, what purpose

---

## Revision History

| Version | Date | Changes | Author |
|---------|------|---------|--------|
| 1.0 | Dec 2025 | Initial brand guidelines (pre-launch foundation) | Kris Armstrong |

---

**Document Owner:** Kris Armstrong, Founder
**Last Reviewed:** December 2025
**Next Review:** After logo design complete (Q1 2026), then quarterly

---

*From a tiny seed, a mighty network grows.*

**Mustard Seed Networks**
**The Seed - AI-Powered Network Diagnostics**
