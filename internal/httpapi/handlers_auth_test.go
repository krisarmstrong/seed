...
// setupStatusTestCase defines a test case for setup status endpoint.
type setupStatusTestCase struct {
	name           string
	useDefaultHash bool
	expectSetup    bool
}

// buildSetupStatusConfig creates the appropriate config for a setup status test case.
func buildSetupStatusConfig(tc setupStatusTestCase) *config.Config {
	defaults := testutil.GetTestDefaults()
	if tc.useDefaultHash {
	...
	return testutil.NewConfigBuilder().Build()
}

// verifySetupStatusResponse validates the setup status response.
func verifySetupStatusResponse(t *testing.T, w *httptest.ResponseRecorder, expectSetup bool) {
	t.Helper()
	...
	if resp.NeedsSetup != expectSetup {
		t.Errorf("Expected needsSetup=%v, got %v", expectSetup, resp.NeedsSetup)
	}

	verifySuggestedPassword(t, resp.SuggestedPassword, expectSetup)
}

// verifySuggestedPassword checks the suggested password field based on setup state.
func verifySuggestedPassword(t *testing.T, suggestedPassword string, expectSetup bool) {
	t.Helper()

	hasSuggestion := suggestedPassword != ""
	if expectSetup && !hasSuggestion {
...
func TestHandleSetupStatus(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			verifySetupStatusResponse(t, w, tt.expectSetup)
		})
	}
}
