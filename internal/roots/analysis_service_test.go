...
// TestScoreToDescription validates score to description conversion.
...
// TestPathAnalysis_Fields validates PathAnalysis struct fields.
func TestPathAnalysis_Fields(t *testing.T) {
	...
	if len(analysis.Bottlenecks) != 1 {
		t.Errorf("len(Bottlenecks) = %d, want 1", len(analysis.Bottlenecks))
	}
	if analysis.Analysis != "Good quality" {
		t.Errorf("Analysis = %q, want %q", analysis.Analysis, "Good quality")
	}
...
// TestPathBottleneck_Fields validates PathBottleneck struct fields.
...
// TestTracerouteHop_Fields validates TracerouteHop struct fields.
