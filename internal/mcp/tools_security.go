...
func (s *Server) registerSecurityTools(isAllowed func(string) bool) {
	...
	s.addTool("vulnerability_scan", isAllowed,
		mcp.NewTool("vulnerability_scan",
			mcp.WithDescription("Scan a device for known vulnerabilities (CVEs) based on detected services and versions. Requires the device to be in the discovered devices list."),
			mcp.WithString("ip",
				mcp.Required(),
	...
	s.addTool("rogue_dhcp_check", isAllowed,
		mcp.NewTool("rogue_dhcp_check",
			mcp.WithDescription("Check for rogue DHCP servers on the network. Returns a list of detected DHCP servers and whether they are known/authorized or potentially rogue."),
		),
	...
	s.addTool("snmp_query", isAllowed,
		mcp.NewTool("snmp_query",
			mcp.WithDescription("Query a device via SNMP to get system information. Uses configured community strings or SNMPv3 credentials."),
			mcp.WithString("host",
				mcp.Required(),
...
func (s *Server) handleVulnerabilityScan(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if scanner == nil {
		return mcp.NewToolResultError("Vulnerability scanner not available. Enable vulnerability scanning in configuration."), nil
	}
...
func (s *Server) handleRogueDHCPCheck(_ context.Context, _ mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if servers == nil {
		return mcp.NewToolResultText("No DHCP servers detected. The detector may need more time to capture DHCP traffic."), nil
	}
...
func (s *Server) handleSNMPQuery(_ context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if err != nil {
		return mcp.NewToolResultError("host parameter is required"), nil //nolint:nilerr // MCP returns tool error, not Go error
	}
